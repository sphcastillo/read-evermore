...
type ExportEntry = {title?: string; author?: string; kind: string; addedAt?: string; finishedAt?: string; readCount?: number; isbn10?: string; isbn13?: string}
...
export async function GET() {
  try {
    ...
    const rows = [['Title', 'Author', 'Exclusive Shelf', 'Date Added', 'Date Read', 'Read Count', 'ISBN', 'ISBN13'],
      ...entries.map((entry) => [entry.title, entry.author, shelves[entry.kind], entry.addedAt?.slice(0, 10), entry.finishedAt, entry.readCount, entry.isbn10, entry.isbn13])]
